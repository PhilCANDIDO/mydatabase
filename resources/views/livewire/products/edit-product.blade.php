<div class="p-6 bg-white border-b border-gray-200">
    <form wire:submit="update">
        ...
        <div class="mb-6">
            ...
            <div class="mb-4">
                <label for="product_family" class="block text-sm font-medium text-gray-700">{{ __('Famille de produit') }}</label>
                <input type="text" id="product_family" value="{{ $product->productFamily->product_family_name }}" class="mt-1 block w-full rounded-md border-gray-300 bg-gray-100 shadow-sm sm:text-sm" disabled>
                <p class="mt-1 text-xs text-gray-500">{{ __('La famille de produit ne peut pas être modifiée.') }}</p>
            </div>
            
            <div class="mb-4">
                <label for="product_type" class="block text-sm font-medium text-gray-700">{{ __('Type') }}</label>
                <input type="text" id="product_type" value="{{ $product_type }}" class="mt-1 block w-full rounded-md border-gray-300 bg-gray-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" disabled>
                <p class="mt-1 text-xs text-gray-500">{{ __('Le type ne peut pas être modifié.') }}</p>
            </div>
            ...
            <!-- Upload de l'avatar -->
            <div class="mb-4">
                <label for="new_product_avatar" class="block text-sm font-medium text-gray-700">{{ __('Avatar du produit') }}</label>
                
                @if($current_avatar)
                    <div class="mt-2 mb-4">
                        <p class="text-sm text-gray-600 mb-2">{{ __('Avatar actuel:') }}</p>
                        <img src="{{ asset('storage/' . $current_avatar) }}" class="h-20 w-20 object-cover rounded-lg shadow">
                    </div>
                @endif
                
                <input type="file" id="new_product_avatar" wire:model="new_product_avatar" class="mt-1 block w-full" accept="image/*">
                <p class="mt-1 text-xs text-gray-500">{{ __('Laissez vide pour conserver l\'avatar actuel.') }}</p>
                @error('new_product_avatar') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                
                @if($new_product_avatar)
                    <div class="mt-2">
                        <p class="text-sm text-gray-600 mb-2">{{ __('Nouvel avatar:') }}</p>
                        <img src="{{ $new_product_avatar->temporaryUrl() }}" class="h-20 w-20 object-cover rounded-lg shadow">
                    </div>
                @endif
            </div>
